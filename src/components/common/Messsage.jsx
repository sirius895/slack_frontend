...
const Message = (props) => {
  ...
  return (
    <VStack
      ...
      {...etcProps}
    >
      <HStack w={"full"} gap={4} onMouseOver={() => setToolShow(true)} onMouseLeave={() => setToolShow(false)}>
        <VStack w={"72px"} h={"full"} justify={"center"} alignItems={"center"}>
          <UserAvatar />
          <Text fontWeight={"extrabold"} color={"var(--mainColor)"}>
            {message.sender.username ? message.sender.username : "Unknown"}
          </Text>
        </VStack>
        <VStack w={"full"} minH={"80px"} gap={2} pl={2} justifyContent={"flex-start"}>
          ...
          <HStack w={"full"} flex={"1 1 0"} alignItems={"flex-start"}>
            <Text w={"full"} whiteSpace={"normal"} flexWrap={"wrap"} fontFamily={"cursive"}>
              {message.content}
            </Text>
          </HStack>
          <HStack w={"full"} gap={2} flexWrap={"wrap"}>
          ...
          <HStack w={"full"} gap={"2"}>
            {message.files.map((m, i) => (
              <FileMark filename={m.filename} originalname={m.originalname} />
            ))}
          </HStack>
        </VStack>
                ...
                <>
                  ...
                  <FaCommentDots
                    onClick={() => {
                      ...
                      navigate(`/chatting/home/${channel}/${message._id}`);
                    }}
                  />
                </>
              )}
